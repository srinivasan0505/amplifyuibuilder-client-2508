...
func ExportcomponentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/export/app/%s/environment/%s/components%s", cfg.BaseURL, appId, environmentName, queryString)
		...
		var result models.ExportComponentsResponse
...
func CreateExportcomponentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_export_app_appId_environment_environmentName_components",
		mcp.WithDescription("Exports component configurations to code that is ready to integrate into an Amplify app."),
		...
		mcp.WithString("appId", mcp.Required(), mcp.Description("The unique ID of the Amplify app to export components to.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ExportcomponentsHandler(cfg),
	}
}
