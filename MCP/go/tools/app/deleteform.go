...
func DeleteformHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteformTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_app_appId_environment_environmentName_forms_id",
		mcp.WithDescription("Deletes a form from an Amplify app."),
		...
		mcp.WithString("appId", mcp.Required(), mcp.Description("The unique ID of the Amplify app associated with the form to delete.")),
		...
		mcp.WithString("id", mcp.Required(), mcp.Description("The unique ID of the form to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteformHandler(cfg),
	}
}
