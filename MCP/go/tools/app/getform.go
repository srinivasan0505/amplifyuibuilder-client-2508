...
func GetformHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/app/%s/environment/%s/forms/%s", cfg.BaseURL, appId, environmentName, id)
		...
		var result models.GetFormResponse
...
func CreateGetformTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_app_appId_environment_environmentName_forms_id",
		mcp.WithDescription("Returns an existing form for an Amplify app."),
		...
		mcp.WithString("id", mcp.Required(), mcp.Description("The unique ID of the form.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetformHandler(cfg),
	}
}
