...
func PutmetadataflagHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		featureNameVal, ok := args["featureName"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: featureName"), nil
		}
		featureName, ok := featureNameVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: featureName"), nil
		}
		// Create properly typed request body using the generated schema
		var requestBody models.PutMetadataFlagrequest
		...
		url := fmt.Sprintf("%s/app/%s/environment/%s/metadata/features/%s", cfg.BaseURL, appId, environmentName, featureName)
...
func CreatePutmetadataflagTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_app_appId_environment_environmentName_metadata_features_featureName",
		mcp.WithDescription("Stores the metadata information about a feature on a form."),
		...
		mcp.WithString("environmentName", mcp.Required(), mcp.Description("The name of the backend environment that is part of the Amplify app.")),
		mcp.WithString("featureName", mcp.Required(), mcp.Description("The name of the feature associated with the metadata.")),
		mcp.WithObject("body", mcp.Required(), mcp.Description("Input parameter: Stores the metadata information about a feature on a form.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    PutmetadataflagHandler(cfg),
	}
}
