...
func DeletethemeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletethemeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_app_appId_environment_environmentName_themes_id",
		mcp.WithDescription("Deletes a theme from an Amplify app."),
		...
		mcp.WithString("appId", mcp.Required(), mcp.Description("The unique ID of the Amplify app associated with the theme to delete.")),
		...
		mcp.WithString("id", mcp.Required(), mcp.Description("The unique ID of the theme to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletethemeHandler(cfg),
	}
}
