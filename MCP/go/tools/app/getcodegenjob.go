...
func GetcodegenjobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/app/%s/environment/%s/codegen-jobs/%s", cfg.BaseURL, appId, environmentName, id)
		...
		var result models.GetCodegenJobResponse
...
func CreateGetcodegenjobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_app_appId_environment_environmentName_codegen-jobs_id",
		mcp.WithDescription("Returns an existing code generation job."),
		...
		mcp.WithString("appId", mcp.Required(), mcp.Description("The unique ID of the Amplify app associated with the code generation job.")),
		mcp.WithString("environmentName", mcp.Required(), mcp.Description("The name of the backend environment that is a part of the Amplify app associated with the code generation job.")),
		mcp.WithString("id", mcp.Required(), mcp.Description("The unique ID of the code generation job.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetcodegenjobHandler(cfg),
	}
}
