...
func CreatethemeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateThemerequest
		...
		url := fmt.Sprintf("%s/app/%s/environment/%s/themes%s", cfg.BaseURL, appId, environmentName, queryString)
		...
		var result models.CreateThemeResponse
...
func CreateCreatethemeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_app_appId_environment_environmentName_themes",
		mcp.WithDescription("Creates a theme to apply to the components in an Amplify app."),
		...
		mcp.WithString("appId", mcp.Required(), mcp.Description("The unique ID of the Amplify app associated with the theme.")),
		...
		mcp.WithObject("themeToCreate", mcp.Required(), mcp.Description("Input parameter: Represents all of the information that is required to create a theme.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatethemeHandler(cfg),
	}
}
