...
func StartcodegenjobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.StartCodegenJobrequest
		...
		url := fmt.Sprintf("%s/app/%s/environment/%s/codegen-jobs%s", cfg.BaseURL, appId, environmentName, queryString)
		...
		var result models.StartCodegenJobResponse
...
func CreateStartcodegenjobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_app_appId_environment_environmentName_codegen-jobs",
		mcp.WithDescription("Starts a code generation job for a specified Amplify app and backend environment."),
		...
		mcp.WithString("clientToken", mcp.Description("The idempotency token used to ensure that the code generation job request completes only once.")),
		mcp.WithObject("codegenJobToCreate", mcp.Required(), mcp.Description("Input parameter: The code generation job resource configuration.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    StartcodegenjobHandler(cfg),
	}
}
