...
func UpdatethemeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateThemerequest
		...
		url := fmt.Sprintf("%s/app/%s/environment/%s/themes/%s%s", cfg.BaseURL, appId, environmentName, id, queryString)
		...
		var result models.UpdateThemeResponse
...
func CreateUpdatethemeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_app_appId_environment_environmentName_themes_id",
		mcp.WithDescription("Updates an existing theme."),
		...
		mcp.WithString("id", mcp.Required(), mcp.Description("The unique ID for the theme.")),
		mcp.WithString("clientToken", mcp.Description("The unique client token.")),
		mcp.WithObject("updatedTheme", mcp.Required(), mcp.Description("Input parameter: Saves the data binding information for a theme.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatethemeHandler(cfg),
	}
}
