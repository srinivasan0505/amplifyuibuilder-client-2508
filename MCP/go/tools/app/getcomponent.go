...
func GetcomponentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/app/%s/environment/%s/components/%s", cfg.BaseURL, appId, environmentName, id)
		...
		var result models.GetComponentResponse
...
func CreateGetcomponentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_app_appId_environment_environmentName_components_id",
		mcp.WithDescription("Returns an existing component for an Amplify app."),
		...
		mcp.WithString("id", mcp.Required(), mcp.Description("The unique ID of the component.")),
	)
