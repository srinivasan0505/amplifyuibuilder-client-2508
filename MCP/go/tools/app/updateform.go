...
func UpdateformHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateFormrequest
		...
		url := fmt.Sprintf("%s/app/%s/environment/%s/forms/%s%s", cfg.BaseURL, appId, environmentName, id, queryString)
		...
		var result models.UpdateFormResponse
...
func CreateUpdateformTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_app_appId_environment_environmentName_forms_id",
		mcp.WithDescription("Updates an existing form."),
		...
		mcp.WithString("id", mcp.Required(), mcp.Description("The unique ID for the form.")),
		mcp.WithString("clientToken", mcp.Description("The unique client token.")),
		mcp.WithObject("updatedForm", mcp.Required(), mcp.Description("Input parameter: Updates and saves all of the information about a form, based on form ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdateformHandler(cfg),
	}
}
