...
func ExchangecodefortokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		providerVal, ok := args["provider"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: provider"), nil
		}
		provider, ok := providerVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: provider"), nil
		}
		// Create properly typed request body using the generated schema
		var requestBody models.ExchangeCodeForTokenrequest
		...
		url := fmt.Sprintf("%s/tokens/%s", cfg.BaseURL, provider)
		...
		var result models.ExchangeCodeForTokenResponse
...
func CreateExchangecodefortokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tokens_provider",
		mcp.WithDescription("Exchanges an access code for a token."),
		...
		mcp.WithString("provider", mcp.Required(), mcp.Description("The third-party provider for the token. The only valid value is <code>figma</code>.")),
		mcp.WithObject("request", mcp.Required(), mcp.Description("Input parameter: Describes the configuration of a request to exchange an access code for a token.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ExchangecodefortokenHandler(cfg),
	}
}
